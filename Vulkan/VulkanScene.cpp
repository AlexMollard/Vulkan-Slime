    ...
    engine->immediate_submit([&](VkCommandBuffer cmd)
                             {
                                 for (auto& m : meshes)
                                 {
                                     ...
                                     vkCmdCopyBuffer(cmd, m.original->mVertexBuffer.mBuffer, mergedVertexBuffer.mBuffer, 1, &vertexCopy);
                                     ...
                                     vkCmdCopyBuffer(cmd, m.original->mIndexBuffer.mBuffer, mergedIndexBuffer.mBuffer, 1, &indexCopy);
                                 }
                             });
}
...
Handle<DrawMesh> RenderScene::getMeshHandle(Mesh* m)
{
    ...
    if (it == meshConvert.end())
    {
        ...
        newMesh.vertexCount = static_cast<uint32_t>(m->mVertices.size());
        newMesh.indexCount = static_cast<uint32_t>(m->mIndices.size());
